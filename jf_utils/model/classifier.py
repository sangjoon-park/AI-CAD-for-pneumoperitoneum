...
from jf_utils.model.backbone.vgg import (vgg19, vgg19_bn)
from jf_utils.model.backbone.densenet import (densenet121, densenet169, densenet201)
from jf_utils.model.backbone.inception import (inception_v3)
from jf_utils.model.global_pool import GlobalPool
from jf_utils.model.attention_map import AttentionMap
...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # feat_map = torch.nn.Sequential(*(list(self.backbone.children())[0][:-2]))(x)
        # [(N, 1), (N,1),...]
        logits = list()
        # [(N, H, W), (N, H, W),...]
        logit_maps = list()
        weighted_feat_maps = list()
        for index, num_class in enumerate(self.cfg.num_classes):
            ...
            if not (self.cfg.global_pool == 'AVG_MAX' or
                ...
                # logit_maps.append(logit_map.squeeze()) # (N, H, W)
                logit_maps.append(logit_map)  # (N, H, W)
            # (N, C, 1, 1)
            feat, weighted_feat_map = self.global_pool(feat_map, logit_map)
            weighted_feat_maps.append(weighted_feat_map)
        ...
        # return torch.sigmoid(torch.cat(logit_maps[1:],dim=1)).float(), torch.cat([weighted_feat_maps[x] for x in selected],dim=1).float()
        return torch.sigmoid(torch.cat(logit_maps[1:],dim=1)).float(), feat_map.float()