...
def getAttMap(img, attMap, probability, blur = True, overlap = True):
    attMap -= attMap.min()
    if attMap.max() > 0:
        attMap /= attMap.max()
    ...
    if blur:
        ...
        attMap -= attMap.min()
        attMap /= attMap.max()
    attMap = attMap * probability
    cmap = plt.get_cmap('jet')
    attMapV = cmap(attMap)
    attMapV = np.delete(attMapV, 3, 2)
    if overlap:
        attMap = 1*(1-attMap**0.95).reshape(attMap.shape + (1,))*img + (attMap**0.95).reshape(attMap.shape+(1,)) * attMapV
    return attMap