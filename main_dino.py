...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='vit_small', type=str,
                        choices=['vit_tiny', 'vit_small', 'vit_base', 'xcit', 'deit_tiny', 'deit_small', 'deit'] \
    ...
    parser.add_argument('--momentum_teacher', default=0.9998, type=float, help="""Ba``se EMA
    ...
    parser.add_argument("--lr", default=0.00004, type=float, help="""Learning rate at the end of
    ...
    # Misc
    parser.add_argument('--output_dir', default="/COVID_8TB/sangjoon/Abdomen/checkpoint/20220511_experiments/20220506_self_supervised/", type=str,
                        help='Path to save logs and checkpoints.')
    # parser.add_argument('--output_dir', default="/mnt/ssd8/sangjoon/checkpoints/20210923_Experiments/",
    ...
    # Required parameters
    parser.add_argument("--pretrain", type=str, default="DINO_pretrained",  # Pre-training option
                        help="Which pretrained weights to select = [jf_none, jf_self_supervised, jf_pretrained, "
                             "jf_self_trained, DINO_none, DINO_self_supervised, DINO_pretrained, DINO_self_trained]")

    parser.add_argument("--data_train", type=str, default="/COVID_8TB/sangjoon/Abdomen/AI_Pneumo_peritoneum/data/train/",
                        help="customized_data_dir")
    parser.add_argument("--data_test", type=str, default="/COVID_8TB/sangjoon/Abdomen/AI_Pneumo_peritoneum/data/val_final/",
                        help="customized_data_dir")
    ...
    parser.add_argument("--jf_model_path", type=str, default="jf_utils/pretrained/jf_best.ckpt",
                        help="customized_data_dir")
    parser.add_argument('--jf_frozen', default=False, type=bool,
                        help="freeze jf_model pre-traiend weight")
    ...
    parser.add_argument('--ruin', default=False, type=bool,         # Works?
                        help="whether to ruin label or not")

    # View options
    parser.add_argument("--view", type=str, default="all", help="X-ray view")
...
def train_dino(args):
    ...
    dataset = Abdomen_dataset.CXR_Dataset(args.view, args.data_train, size=1.0, transforms=transform, mode='train', labeled=False)
    ...
    l_dataset = Abdomen_dataset.CXR_Dataset(args.view, args.data_train, size=1.0, transforms=transform, mode='train', labeled=True)
    ...
    # # Fix pre-trained CNN backbone
    # if args.use_original == False:
    #     for p in student.module.backbone.hybrid_model.parameters():
    #         p.requires_grad = False
    # print("CNN backbone weights fixed.")
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, bce_loss, data_loader, l_data_loader,
    ...
    for it, (images, _, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if it % args.correct == 0:
            ...
            for _, (images, labels, _) in enumerate(metric_logger.log_every(l_data_loader, 10, header)):
                # move images to gpu
                images = [im.cuda(non_blocking=True) for im in images]
                ...
                with torch.cuda.amp.autocast(fp16_scaler is not None):
                    ...
                    # else:
                    loss = l_loss_ce * 1.0 + l_loss_dino * 0.0
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        self.global_transfo2 = transforms.Compose([
            ...
            utils.GaussianBlur(0.2, radius_min=0.1, radius_max=1.0),
            normalize,
        ])
        ...
        self.local_transfo = transforms.Compose([
            ...
            utils.GaussianBlur(0.5, radius_min=0.5, radius_max=2.0),
            normalize,
        ])
