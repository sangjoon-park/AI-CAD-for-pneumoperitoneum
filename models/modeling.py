...
ACT2FN = {"gelu": torch.nn.functional.gelu, "relu": torch.nn.functional.relu, "swish": swish, "iden": nn.Identity()}
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.head_0 = Linear(config.hidden_size, 2)

    def forward(self, x):
        x, attn_weights = self.transformer(x)
        logits = self.head_0(x[:, 0])
    ...
    def load_from(self, weights):
        with torch.no_grad():
            if self.zero_head:
                nn.init.zeros_(self.head_0.weight)
                nn.init.zeros_(self.head_0.bias)
                nn.init.zeros_(self.head_1.weight)
                nn.init.zeros_(self.head_1.bias)
                nn.init.zeros_(self.head_2.weight)
                nn.init.zeros_(self.head_2.bias)
            else:
